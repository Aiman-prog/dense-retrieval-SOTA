...
# Sequential BRIGHT eval of a parallel Fast-GRASS sweep (trained with --no_eval).
# Runs ONE checkpoint at a time, so the shared eval scratch dir
# (temp_grass_workdir/final_eval/...) never races. Point it at the swept dirs:
#   FG_EVAL_GLOB="models/fg_*"  sbatch scripts/run_fast_grass_eval_singularity.sh
...
FG_EVAL_GLOB="${FG_EVAL_GLOB:-${DATA_BASE_DIR}/models/fg_*}"
